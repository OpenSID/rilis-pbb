                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                <!-- Tombol Tambah Data -->
                                <a href="{{ route('pengguna.create') }}" class="btn btn-success"><i
                                        class="fa fa-plus-circle me-2"></i>Tambah</a>

                                <!-- Tombol Hapus Data Yang Dipilih -->
                                <button type="button" class="btn btn-sm btn-danger btn-hapus-data-dipilih"
                                    id="deleteAllBtn" data-bs-toggle="modal"
                                    data-bs-target="#hapusDataDipilih-{{ $table }}" disabled>
                                        ...
                                        @foreach ($pengguna as $index => $item)
                                            <tr id="sid{{ $item->id }}">
                                                <td class="text-center"><input type="checkbox" name="ids"
                                                        class="checkBoxClass" value="{{ $item->id }}"></td>
                                                ...
                                                <td class="text-center">
                                                    @if ($item->photo)
                                                        <img src="{{ asset('storage/pengguna/' . $item->photo) }}"
                                                            alt="" class="thumbnail image-responsive">
                                                    @endif
                                                </td>

                                                <td class="text-center">
                                                    <!-- Tombol Ubah Data -->
                                                    <a href="{{ route('pengguna.edit', encrypt($item->id)) }}"
                                                        class="btn btn-primary btn-sm {{ $item->id == 1 && $demo == true ? 'd-none' : '' }}">
                                                        <i class="fa fa-pencil"></i>
                                                    </a>

                                                    <!-- Tombol Hapus Data -->
                                                    <button type="button"
                                                        class="btn btn-sm btn-danger {{ $item->id == 1 && $demo == true ? 'd-none' : '' }}"
