...
test.describe('Rayon Tests', () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to rayon page after authentication
  ...
  test('should display rayon page title and basic layout', async ({ page }) => {
    // Check specific page title for rayon
    await expect(page).toHaveTitle('Rayon | Pencatatan Pajak');

    // Check main content container
    await expect(page.locator('.content')).toBeVisible();

    // Check for rayon-specific heading
    await expect(page.locator('h1:has-text("Rayon"), h2:has-text("Rayon"), h3:has-text("Rayon")')).toBeVisible();
  });

  test('should display rayon data table', async ({ page }) => {
    // Check if there are multiple tables as expected (11 tables found)
    const tables = page.locator('table, .table');
    await expect(tables).toHaveCount(11);

    // Check that at least the first table is visible
  ...
  test('should display action buttons if available', async ({ page }) => {
    // Check for the "Tambah" (Add) button specifically found in the structure
    const addButton = page.locator('button:has-text("Tambah"), a:has-text("Tambah"), .btn:has-text("Tambah")');
    await expect(addButton).toBeVisible();

    // Check for edit buttons (might be in table rows)
    const editButtons = page.locator('button:has-text("Edit"), a:has-text("Edit"), .btn:has-text("Edit")');
    const editCount = await editButtons.count();

    // Check for delete confirmation dialogs (found multiple in structure)
    const deleteConfirmations = page.locator('text="Apakah Anda yakin menghapus data rayon yang dipilih ?"');
    // These are likely hidden modal content, so we just check they exist in DOM
    expect(await deleteConfirmations.count()).toBeGreaterThan(0);

    // Check for "Data SPPT Tanggungan Rayon" links
    const spptLinks = page.locator('text="Data SPPT Tanggungan Rayon"');
    expect(await spptLinks.count()).toBeGreaterThan(0);

    // Check for "Rekap Pembayaran" links
    const rekapLinks = page.locator('text="Rekap Pembayaran"');
    expect(await rekapLinks.count()).toBeGreaterThan(0);
  });

  test('should display rayon cards or list items', async ({ page }) => {
    // Check for cards (found 2 cards in the actual page)
    const cards = page.locator('.card, .card-body');
    await expect(cards).toHaveCount(2);
    await expect(cards.first()).toBeVisible();

    // Check for table rows which act as list items for rayon data
  ...
  test('should display specific rayon functionality', async ({ page }) => {
    // Test rayon-specific elements found in the page structure

    // Check for rayon action links
    const spptTanggunganLinks = page.locator('text="Data SPPT Tanggungan Rayon"');
    const rekapPembayaranLinks = page.locator('text="Rekap Pembayaran"');

    // These should exist in the page (found multiple instances)
    expect(await spptTanggunganLinks.count()).toBeGreaterThan(0);
    expect(await rekapPembayaranLinks.count()).toBeGreaterThan(0);

    // Check that tables contain rayon data
    const tables = page.locator('table');
    await expect(tables).toHaveCount(11);

    // Check for the add button with a more specific selector
    const tambahButton = page.locator('button:has-text("Tambah"), .btn:has-text("Tambah")').first();
  ...
  test('should handle tambah button click functionality', async ({ page }) => {
    ...
    if (await tambahButton.count() > 0) {
      ...
      if (newUrl !== currentUrl) {
        ...
        // Verify page has form elements or create content
        ...
        expect(hasCreateForms || hasAnyForms || hasCreateContent).toBeTruthy();
      } else {
        // No navigation occurred - button click was successful but no redirect
  ...
  test('should be responsive and have proper CSS classes', async ({ page }) => {
    // Check for Bootstrap column classes more specifically
