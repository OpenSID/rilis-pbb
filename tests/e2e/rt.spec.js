...
test.describe('RT Tests', () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to RT page after authentication
    await page.goto('/rt');
  ...
  test('should display RT page title and basic layout', async ({ page }) => {
    // Check specific page title for RT
    await expect(page).toHaveTitle(/RT|Rukun Tetangga|PBB/);

    // Check main content container
    await expect(page.locator('.content')).toBeVisible();

    // Check for RT-specific heading
    await expect(page.locator('h1:has-text("RT"), h2:has-text("RT"), h3:has-text("RT"), h1:has-text("Rukun Tetangga"), h2:has-text("Rukun Tetangga"), h3:has-text("Rukun Tetangga")')).toBeVisible();
  });

  test('should display RT data table', async ({ page }) => {
  ...
  test('should display action buttons if available', async ({ page }) => {
    ...
    // Check for delete buttons or confirmations
  ...
  test('should display RT cards or list items', async ({ page }) => {
    // Check for cards
    ...
    // Check for table rows which act as list items for RT data
  ...
  test('should handle tambah button click functionality', async ({ page }) => {
    ...
    if (await tambahButton.count() > 0) {
      ...
      if (newUrl !== currentUrl) {
        ...
        // Check for cancel/back button or link
        const backButton = page.locator('button:has-text("Kembali"), a:has-text("Kembali"), button:has-text("Batal"), a:has-text("Batal")');
